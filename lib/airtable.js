...
async function getRecord(tableId, recordId) {
  if (!tableId) throw new Error('getRecord called with no tableId — check config/env vars');

  const res = await fetch(`${BASE_URL}/${config.airtable.baseId}/${tableId}/${recordId}`, {
    headers: authHeaders(),
  });

  if (!res.ok) {
    const body = await res.text();
    throw new Error(`Airtable getRecord failed (${res.status}) on table ${tableId}: ${body}`);
  }

  return res.json();
}

module.exports = { listRecords, createRecord, updateRecords, deleteRecords, getRecord };
